...
Feature: delete groups
  ...
  Scenario: admin deletes a group that has a forward-slash in the group name
    ...
    When the administrator sends a group creation request for the following group using the provisioning API
      | groupname        | comment                            |
    ...
    And the administrator deletes the following groups using the provisioning API
      | groupname        |
      | Mgmt/Sydney      |
      | Mgmt//NSW/Sydney |
      | var/../etc       |
